...
int main() {
    ...
    do {
        ...
        if (getWeatherData.status_code != 200 || getWeatherData2.status_code != 200) {
            std::cerr << "Failed to retrieve weather data. Please check the city name and try again." << std::endl;
            city.clear(); // Clear city to prompt user again
            continue;
        }
        ...
        try {
            ...
            std::cerr << "Failed to parse weather data: " << e.what() << std::endl;
            city.clear(); // Clear city to prompt user again
            continue;
        }
